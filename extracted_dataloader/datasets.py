...
class MIMICCXR(Dataset):
    def __init__(self, paths, args, transform=None, split="train"):
        ...
        self.filesnames_to_labels = dict(
            zip(metadata_with_labels["dicom_id"].values, metadata_with_labels[self.CLASSES].values)
        )
        self.filenames_loaded = splits.loc[splits.split == split]["dicom_id"].values
        self.transform = transform
        self.filenames_loaded = [
            filename for filename in self.filenames_loaded if filename in self.filesnames_to_labels
        ]
...
class MIMIC_CXR_EHR(Dataset):
    def __init__(self, args, metadata_with_labels, ehr_ds, cxr_ds, split="train"):
        ...
        self.get_len = {
            "paired": len(self.ehr_files_paired),
