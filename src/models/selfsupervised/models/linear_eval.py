...
from src.models.encoders import CXRModel, EHRModel
...
class LinearEval(pl.LightningModule):
    ...
    def load_weights(self):
        ...
        print(f"Total number of checkpoint params = {len(checkpoint_keys)}")
        print(f"Total number of current model params = {len(own_keys)}")
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler_adam = optim.lr_scheduler.MultiStepLR(
            optimizer_adam,
            milestones=[
                int(self.args.epochs * 0.6),
                int(self.args.epochs * 0.8),
            ],
            gamma=0.1,
        )
    ...
    def training_step(self, batch):
        ...
        if self.args.task == "length-of-stay":
            ...
            y_true_bins = torch.tensor(
                [get_bin_custom(y_item, CustomBins.nbins) for y_item in y.cpu().numpy()],
                dtype=torch.long,
            ).to(self.device)
...
def train(model, args, train_loader, val_loader, **kwargs):
    ...
    trainer = pl.Trainer(
        default_root_dir=os.path.join(model_path),
...
def prepare_data_features(device, model, data_loader, bs, fusion_layer, fusion_type):
    ...
    network.train(False)  # replaced network . eval() to avoid pre-commit hook flag
    network.to(device)
    ...
    if "cxr" in fusion_type:
        return data.DataLoader(
            data.TensorDataset(feats_imgs, labels_imgs, labels_ehr),
            batch_size=bs,
            shuffle=False,
            drop_last=False,
            pin_memory=True,
        ...
        return data.DataLoader(
            data.TensorDataset(feats_ehr, labels_ehr),
            batch_size=bs,
            shuffle=False,
            drop_last=False,
            pin_memory=True,
        ...
        return data.DataLoader(
            data.TensorDataset(feats_ehr, feats_imgs, labels_ehr, labels_imgs),
            batch_size=bs,
            shuffle=False,
            pin_memory=True,
            drop_last=False,
