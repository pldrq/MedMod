...
from src.data.utils import get_cxr_datasets, get_datasets, load_cxr_ehr
from src.models.selfsupervised.models.linear_eval import LinearEval, prepare_data_features, test, train
...
def initiate_logger(tags):
    logger = pl_loggers.NeptuneLogger(
        project="shaza-workspace/mml-ssl",
        api_key="eyJhcGlfYWRkcmVzcyI6Imh0dHBzOi8vYXBwLm5lcHR1bmUuYWkiLCJhcGlfdXJsIjoiaHR0cHM6Ly9hcHAubmVwdHVuZS5haSIsImFwaV9rZXkiOiI4NDU3ZDlmMi01OGEyLTQzMTAtODJmYS01Mjc5N2U4ZjgyMTAifQ==",
        tags=tags,
...
def load_weights(model, path):
    ...
    for name in own_keys:
        if name not in checkpoint_keys:
            ...
            own_state[name].copy_(param)
            count += 1
    print(f"Total number params loaded for model weights = {count}")
...
if __name__ == "__main__":
    ...
    print(load_dir_simclr + f"/{args.load_state}/*")
    paths_models = glob.glob(load_dir_simclr + f"/{args.load_state}/*")
    ...
    args.file_name = args.file_name + f"-lr{args.lr}-e{args.epochs}-bs{args.batch_size}"
    ...
    for i in range(start_epoch, num_epochs):
        ...
        if ("lineareval" in args.fusion_type) & (not args.finetune):
            ...
            train_dl_pr = prepare_data_features(
                device,
                model,
                train_dl,
                args.batch_size,
            ...
            val_dl_pr = prepare_data_features(
                device,
                model,
                val_dl,
                args.batch_size,
            ...
            test_dl_pr = prepare_data_features(
                device,
                model,
                test_dl,
                args.batch_size,
    ...
    print(f"Final results for best epoch={best_results.epoch}:", best_results)
