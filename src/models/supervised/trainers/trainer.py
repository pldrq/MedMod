...
class Trainer:
    ...
    def computeAUROC(self, y_true, predictions):
        y_true = np.array(y_true)
        predictions = np.array(predictions)

        auc_scores = metrics.roc_auc_score(y_true, predictions, average=None)
        ave_auc_micro = metrics.roc_auc_score(
            y_true,
            predictions,
            average="micro",
        )
        ave_auc_macro = metrics.roc_auc_score(
            y_true,
            predictions,
            average="macro",
        )
    ...
    def save_checkpoint(self, prefix="best"):
        path = (
            f"{self.args.save_dir}/{prefix}_{self.args.fusion_type}_{self.args.task}_{self.args.lr}_checkpoint.pth.tar"
        )
        torch.save(
            {
                "epoch": self.epoch,
