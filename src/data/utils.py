...
from .datasets import MIMIC_CXR_EHR, MIMICCXR, EHRdataset
...
def get_transforms(args):
    ...
    train_transforms.append(transforms.RandomAffine(degrees=45, scale=(0.85, 1.15), shear=0, translate=(0.15, 0.15)))
...
def get_datasets(discretizer, normalizer, args):
    ...
    train_ds = EHRdataset(
        ...
        discretizer,
        normalizer,
        f"{args.ehr_data_root}/{args.task}/train_listfile.csv",
        os.path.join(args.ehr_data_root, f"{args.task}/train"),
    ...
    val_ds = EHRdataset(
        ...
        discretizer,
        normalizer,
        f"{args.ehr_data_root}/{args.task}/val_listfile.csv",
        os.path.join(args.ehr_data_root, f"{args.task}/train"),
        transforms=transform,
    )
    test_ds = EHRdataset(
        ...
        discretizer,
        normalizer,
        f"{args.ehr_data_root}/{args.task}/test_listfile.csv",
        os.path.join(args.ehr_data_root, f"{args.task}/test"),
        transforms=transform,
    )
...
def pad_zeros(arr, min_length=None):
    ...
    if (min_length is not None) and ret[0].shape[0] < min_length:
        ret = [
            np.concatenate([x, np.zeros((min_length - x.shape[0],) + x.shape[1:], dtype=dtype)], axis=0) for x in ret
        ]
...
def get_all_metadata(args):
    ...
    cxr_merged_icustays["time_diff"] = cxr_merged_icustays["time_diff"].apply(
        lambda x: np.round(x.total_seconds() / 60 / 60, 3),
    )

    cxr_merged_icustays["full_stay_time"] = cxr_merged_icustays.outtime - cxr_merged_icustays.intime
    cxr_merged_icustays["full_stay_time"] = cxr_merged_icustays["full_stay_time"].apply(
        lambda x: np.round(x.total_seconds() / 60 / 60, 3),
    )
...
load_task_meta = {
    "decompensation": load_decompensation_meta,
...
def get_final_meta(args):
    ...
    metadata_with_labels = metadata.merge(labels[["study_id"]], how="inner", on="study_id").drop_duplicates(
        subset=["dicom_id"],
    )
    train_meta_with_labels = train_meta_with_labels.merge(
        metadata_with_labels[["dicom_id"]],
        how="inner",
        on="dicom_id",
    )
